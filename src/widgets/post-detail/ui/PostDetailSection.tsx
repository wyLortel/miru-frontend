...
export function PostDetailSection({ post, postId }: PostDetailSectionProps) {
  ...
  const isOwner = mounted && (user?.nickname === post.writer || user?.role === 'ADMIN');
  ...
  return (
    <div className="max-w-4xl mx-auto py-10 px-4">
      <PostDetailHeader
        ...
        actions={isOwner && (
          <>
            <Button variant="ghost" size="sm" className="cursor-pointer hover:bg-primary/10 hover:text-primary" onClick={() => router.push(`/board/${postId}/edit`)}>수정</Button>
      ...
      <div className="flex justify-start">
        <LikeButton postId={postId} likeCount={post.likeCount} isLiked={post.isLiked} />
      </div>
    </div>
  );
}
