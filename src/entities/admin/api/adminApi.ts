...
import type {
  ...
  AdminInquiryDetail,
  AdminUsersResponse,
  AdminUserBoardsResponse,
  AdminUserCommentsResponse,
} from '../model/types';
...
export const adminApi = {
  ...
  postAnswer: async (id: number, answerContent: string): Promise<void> => {
    await apiClient.post(`/api/admin/inquiries/${id}/answer`, {
      answerContent,
    });
  },
  ...
  getUserBoards: async (
    userId: number,
    page: number,
  ): Promise<AdminUserBoardsResponse> => {
    const res = await apiClient.get(`/api/admin/users/${userId}/boards`, {
      params: { page },
    });
    return res.data.data;
  },
  ...
  getUserComments: async (
    userId: number,
    page: number,
  ): Promise<AdminUserCommentsResponse> => {
    const res = await apiClient.get(`/api/admin/users/${userId}/comments`, {
      params: { page },
    });
    return res.data.data;
  },
