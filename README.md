...
### 5. 反応型デザイン — 実機器ベースの検証
DevTools での検証と実機器での動作が異なることに気づき、検証基準を変更しました。

- **問題**
  - DevTools で 320px は OK に見えるが、実機器では レイアウトが崩れる
- **原因**
  - 実機器では アドレスバー、下部バー、safe area など、DevTools では考慮されない要素がある
  - CSS が想定する viewport と実際の表示領域が一致しない
- **改善**
  - DevTools 中心の検証から **実機器ベースの検証** に変更
  - 単なるピクセル数ではなく、実環境での可視領域に対応するレイアウト設計

> 反応型は「スマホサイズ」の数字を合わせることではなく、  
> 実際の利用環境を考慮した検証が本質です。

---

### 6. OAuth2 直後の CSRF タイミング不具合 — 原因特定と最小修正
